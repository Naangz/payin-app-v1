...
class RecentQuotations extends StatelessWidget {
  ...
  const RecentQuotations({Key? key, required this.controller}) : super(key: key);
      ...
      return Column(
        ...
        children: [
          Row(
            ...
            children: [
              const Text('Quotation Terbaru', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              TextButton(
                onPressed: () async {
              ...
              : Column(
                  children: quotations.map((q) {
                    return ListTile(
                      ...
                      subtitle: Text('Klien: ${q.clientName}\nTanggal: ${q.createdDate.toLocal().toString().split(' ')[0]}'),
                      trailing: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Text(q.formattedTotal),
                          Text(q.status, style: TextStyle(fontSize: 12, color: Colors.grey[600])),
                        ],
                      ),
                    );
                  }).toList(),
